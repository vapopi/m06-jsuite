...
class CopyPlugin {
  ...
  static async runPattern(globby, compiler, compilation, logger, cache, inputPattern, index) {
    ...
    const normalizedOriginalFrom = _path.default.normalize(originalFrom);

    logger.log(`starting to process a pattern from '${normalizedOriginalFrom}' using '${pattern.context}' context`);
    let absoluteFrom;

    if (_path.default.isAbsolute(normalizedOriginalFrom)) {
      absoluteFrom = normalizedOriginalFrom;
    } else {
      absoluteFrom = _path.default.resolve(pattern.context, normalizedOriginalFrom);
    }
    ...
    try {
      stats = await (0, _utils.stat)(inputFileSystem, absoluteFrom);
    } catch (error) {// Nothing
    }
    /**
     * @type {"file" | "dir" | "glob"}
    ...
    switch (fromType) {
      case "dir":
        ...
        glob = _path.default.posix.join(_fastGlob.default.escapePath((0, _normalizePath.default)(_path.default.resolve(absoluteFrom))), "**/*");
      ...
      case "file":
        ...
        pattern.context = _path.default.dirname(absoluteFrom);
        glob = _fastGlob.default.escapePath((0, _normalizePath.default)(_path.default.resolve(absoluteFrom)));
      ...
      default:
        {
          const contextDependencies = _path.default.normalize((0, _globParent.default)(absoluteFrom));

          compilation.contextDependencies.add(contextDependencies);
          logger.debug(`added '${contextDependencies}' as a context dependency`);
          glob = _path.default.isAbsolute(originalFrom) ? originalFrom : _path.default.posix.join(_fastGlob.default.escapePath((0, _normalizePath.default)(_path.default.resolve(pattern.context))), originalFrom);
        }
    }
    ...
    try {
      ...
      async globEntry => {
        ...
        const toType = pattern.toType ? pattern.toType : template.test(to) ? "template" : _path.default.extname(to) === "" || to.slice(-1) === _path.default.sep ? "dir" : "file";
        logger.log(`'to' option '${to}' determinated as '${toType}'`);

        const relativeFrom = _path.default.relative(pattern.context, absoluteFilename);

        let filename = toType === "dir" ? _path.default.join(to, relativeFrom) : to;
        ...
        const sourceFilename = (0, _normalizePath.default)(_path.default.relative(compiler.context, absoluteFilename)); // If this came from a glob or dir, add it to the file dependencies
        ...
        if (pattern.transform) {
          ...
          if (transformObj.transformer) {
            ...
            if (transformObj.cache) {
              ...
              const cacheKeys = `transform|${(0, _serializeJavascript.default)(typeof transformObj.cache === "boolean" ? defaultCacheKeys : typeof transformObj.cache.keys === "function" ? await transformObj.cache.keys(defaultCacheKeys, absoluteFilename) : { ...defaultCacheKeys,
        ...
        if (toType === "template") {
          ...
          const base = _path.default.basename(sourceFilename);
          ...
          const {
            ...
            info: assetInfo
          } = compilation.getPathWithInfo((0, _normalizePath.default)(filename), data);
          info = { ...info,
            ...assetInfo
          };
          ...
          filename = (0, _normalizePath.default)(filename);
        } // eslint-disable-next-line consistent-return
    ...
    compiler.hooks.thisCompilation.tap(pluginName, compilation => {
      ...
      compilation.hooks.processAssets.tapAsync({
        ...
        (item, index) => scheduledTasks.push(async () => {
          ...
          const context = typeof normalizedPattern.context === "undefined" ? compiler.context : _path.default.isAbsolute(normalizedPattern.context) ? normalizedPattern.context : _path.default.join(compiler.context, normalizedPattern.context);
          ...
          if (typeof normalizedPattern.transformAll !== "undefined") {
            ...
            const cacheItem = cache.getItemCache(`transformAll|${(0, _serializeJavascript.default)({
              version: _package.version,
            ...
            if (!transformedAsset) {
              ...
              if (template.test(filename)) {
                ...
                const {
                  path: interpolatedFilename,
                  info: assetInfo
                } = compilation.getPathWithInfo((0, _normalizePath.default)(filename), {
                  contentHash,
                  chunk: {
